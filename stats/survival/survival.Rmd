...
`time = ` refers to the duration of time (hour, day, month, year, etc), and
...
`event = ` a binary indication (0 or 1) of whether or not the event occurred. 

---

# Creating the surv object

First we have our dataset, `survival_basic`, which contains two columns.
...
Once we have created our `Surv` object, `surv_obj`, we can use the function `survfit()` from the **survival** package to fit our first analysis. A *Kaplan Meier (KM)* estimate of the survival curve.
...
ggplot(data = surv_plot_data,
       aes(x = time,
           ...
           ymax = conf.high)) +
  geom_line() +                
  geom_ribbon(alpha = 0.25) +    #This plots the 95% CI information given by ymin and ymax, alpha changes the opacity
  labs(x = "Time",
