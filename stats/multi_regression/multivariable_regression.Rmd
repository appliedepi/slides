...
fitControl <- trainControl(
     ...
     number = 5,            # Divide the dataset into 5 sections for training and testing
...
fitControl <- trainControl(
     method = "repeatedcv", # We want to use repeated cross validation
     number = 5,            # Divide the dataset into 5 sections
     repeats = 10)          # Repeat this with 10 "cuts" of data
```
]
...
2) Then we use `train()` from **caret** to run the lasso regression (in caret we specify `method = "glmnet"`) according to the conditions we specified with `trainControl()`
...
cross_tab <- multivariable_regression_table$inputs$x$data %>%
