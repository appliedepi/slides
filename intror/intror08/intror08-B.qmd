...
# Other symptoms
df_symptoms <- import(here("intror/gi_2021-07-07_symptoms.xlsx")) |> 
  select(-date_onset, -symp_dehydration, -symp_diarrhea) 

df_symptoms[] <- lapply(names(df_symptoms), function(col) { 
  if (grepl("^symp", col)) as.logical(df_symptoms[[col]]) else df_symptoms[[col]]
})

df_symptoms[is.na(df_symptoms)] <- FALSE

### REMOVE DIARRHEA AND DEHYDRATION! REMOVE NAS AS NOT MEANINGFUL ANYWAY, UNLESS ENTIRE ROW IS EMPTY. CHANGE TO TRUE AND FALSE
...
::: incremental
-   You have been asked to better understand the symptoms of cases in
...
df_long <- df_wide |> 
  pivot_longer(
    cols = columns_to_pivot,    # Specify the columns to pivot into long format
    names_to = "new_column_name",  # Name for the new column that holds variable names
    values_to = "new_value_column"  # Name for the new column that holds the values
  )
```
:::
...
You might remember other {stringr} functions like str_to_upper() to capitalise text or str_detect() to evaluate if a specific pattern is present inside a string.

Another function is: `str_replace()`

- *Purpose*: Replace the first instance of a pattern in a string with a specified replacement
- *Why*: To modify or clean parts of text data
...
Let's add some code to the previous chunk that created data for a timeline. We systematically remove the `date_` prefix within the `date_type` column.
...
{dplyr}
:::
See how this improves the legend values, without any changes to the ggplot code!
...
# Other symptoms
df_symptoms <- import(here("data/gi_2021-07-07_symptoms.xlsx"))
...
table_overview |>  
  flextable() |> 
    set_header_labels(         # Rename the columns in original header row
      ...
      latest_report = "Most recent",
      median_age = "Median age",
      max_age = "Max age")   |> 
  add_header_row(
    top = TRUE,                # New header goes on top of existing header row
    values = c("Region",     # Header values for each column below
               "Case counts", 
               "",    # This will be the top-level header for this and two next columns
               "",
               "Report dates",
               "",
               "Age",         # This will be the top-level header for this and two next columns
               ""))  |> 
    merge_at(i = 1, j = 2:4, part = "header") |>  # Horizontally merge columns 3 to 5 in new header row
  merge_at(i = 1, j = 5:6, part = "header") |>     # Horizontally merge columns 6 to 8 in new header row
  merge_at(i = 1, j = 7:8, part = "header")     # Horizontally merge columns 6 to 8 in new header row
