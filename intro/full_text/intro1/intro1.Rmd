...
We are glad that you are taking the time to learn R with us.  
...
A **grassroots**-driven **nonprofit** organization of **170 experienced epidemiologists**.  

**“By epis, for epis”** - R adoption should be led by ground-level users.
]
...
- Used **3,000 times daily** by **650,000 global users**  
...
Used by hundreds of agencies including WHO, MSF, Field Epi Training Programs (FETPs), and the Uganda Ministry of Health for Ebola response.  
...
.pull-right[
*317 COVID-19 reports produced daily by Public Health England in 2020*
...
.footnote[Image source: Paula Bianca Blomquist]
...
.footnote[All course data are not real.]  
...
A typical module is structured as below:  
...
* Understand R, RStudio, and RStudio projects  
* Write basic R code to:  
  * Perform calculations  
  * Save and change objects  
  * Use functions  
...
*In R, you typically must tell R which dataset to use, because you can store multiple*.  
...
This package helps you manage the install/loading of other packages.  
...
In this module you will use the `import()` function from **{rio}** to import data from your **RStudio project**. We will discuss **{here}** in the next module. 
...
Importing data into R is easy with the **`import()`** function from the {rio} package.  

Its primary **argument** expects a **character file path** to the dataset, and uses the file extension (.csv, .xlsx) to understand the data type.  
...
To import a file saved in the project "root" folder, no other information is needed.
...
.footnote[We will address other importing scenarios in the next module]  
...
Open the exercise for Module 1, and login  
Follow the instructions to create an RStudio project and begin coding  
