...
```{r, echo=T, eval = F}
surv <- surv_raw %>%        # define new clean data as: the raw data piped to...
  clean_names() %>%                           # standardize column names 
  rename(                                     # manual column names edits:
    age_years = age,                            # age to age_years
    date_onset = onset_date,                    # date of onset
    date_report = date_of_report) %>%           # date of report
...
The `mutate(sex...` has not received the dataset from the previous step, and therefore R has no context of what `sex` is.  
...
* Keep **one cleaning command** only. Add new functions with pipes to the bottom.  
