...
subtitle: <br>{ggplot2} for donnees visualization
author: ''
...
.footnote[Images sources [ici](http://r-statistics.co/Top50-Ggplot2-Visualizations-MasterList-R-Code.html) et [ici](https://jcheshire.com/r-spatial-donnees-hints/great-maps-ggplot2/)]
...
lines <- surv %>% 
  group_by(hopital,
           week_report = lubridate::floor_date(date_symptomes, "week")) %>% 
  summarise(n_cas = n()) %>% 
  drop_na(week_report) %>% 
  ggplot(
    mapping = aes(
...
```{r, echo=TRUE, eval=FALSE, tidy=FALSE}
ggplot(donnees = surv, mapping = aes(x = age_ans, y = ht_cm)) + geom_point()
```
...
ggplot(
  donnees = surv,        # use case linelist
  mapping = aes(      # make aesthetic mappings for all geoms
    x = age_ans,    # assign x-axis to age column
    y = ht_cm)) +     # assign y-axis to height
geom_point()          # display donnees as points
```

```{r, echo=params$lang == "fr", eval=FALSE, tidy=FALSE}
ggplot(
  donnees = surv,        # utiliser le cas linelist
  mapping = aes(      # faire des mappings esthétiques pour tous les géomètres
    x = age_ans,    # assigner la colonne d'âge à l'axe des x 
...
ggplot(
  donnees = surv,     # использование линейного списка
  mapping = aes(   # создаем эстетические отображения для всех геомов
    x = age_ans, # присвоить ось x столбцу возраста
...
ggplot(
  donnees = surv,        # caso de uso lista de líneas
  mapping = aes(      # hacer mapeos estéticos para todos los geoms
    x = age_ans,    # asignar el eje x a la columna de edad
...
<!-- color_purple <- ggplot(donnees = surv, aes(x = age_ans, y = ht_cm))+ -->
...
<!-- color_hospital <- ggplot(donnees = surv, aes(x = age_ans, y = ht_cm))+ -->

<!-- geom_point(aes(color = hopital))+ -->

<!-- theme_minimal(base_size = 16)+ -->

<!-- labs(title = "Using aes(color = hopital)") -->
...
<!-- labs(title = "Utilisant aes(color = hopital)") -->
...
<!-- labs(title = "Использование aes(color = hopital), больница") -->
...
<!-- labs(title = "Utilizar aes(color = hopital)") -->
...
<!-- fill_purple <- ggplot(donnees = surv, aes(x = date_symptomes))+ -->
...
<!-- fill_hospital <- ggplot(donnees = surv, aes(x = date_symptomes))+ -->

<!-- geom_histogram(aes(fill = hôpital))+ -->

<!-- theme_minimal(base_size = 16)+ -->

<!-- labs(title = "Using aes(fill = hopital)") -->
...
<!-- labs(title = "Utilisant aes(fill = hopital)") -->
...
<!-- geom_histogram(aes(fill = hopital))+ -->

<!-- theme_minimal(base_size = 16)+ -->

<!-- labs(title = "Utilizar aes(fill = hopital)") -->
...
`color=` est affectée à la colonne `hopital` à l'intérieur de `aes()`.
...
ggplot(
  ...
  mapping = aes(
    x = age_ans,
    y = ht_cm,
    color = hopital))+      # color varies by resultat
...
surv %>% 
  # get weekly compte by hopital
  group_by(
     hopital,
     week = floor_date(date_symptomes, "week")
     ) %>%
  count() %>% 
...
surv %>% 
  # Obtenga los recuentos semanales por hopital
  group_by(
     hopital,
     week = floor_date(date_symptomes, "week")) %>%
  count() %>% 
...
surv %>% 
  ...
  ggplot(
    mapping = aes(
      ...
      color = hopital)) +
  geom_line()+
  gghighlight::gghighlight(hopital == "Port Hospital") +
  theme(legend.position = "none")+
  theme_light(base_size = 16)+
  labs(title = "Weekly cases by hopital", subtitle = "Port Hospital highlighted")
```
