...
pacman::p_load(xaringan, gghighlight)
...
```{r, echo=TRUE, eval=FALSE, tidy=FALSE}
ggplot(data = surv, mapping = aes(x = age_ans, y = ht_cm)) + geom_point()
```
...
<!-- color_purple <- ggplot(data = surv, aes(x = age_ans, y = ht_cm))+ -->
...
<!-- color_hospital <- ggplot(data = surv, aes(x = age_ans, y = ht_cm))+ -->
...
<!-- fill_purple <- ggplot(data = surv, aes(x = date_symptomes))+ -->
...
<!-- fill_hospital <- ggplot(data = surv, aes(x = date_symptomes))+ -->
...
**Statique :** valeurs esthétiques **numériques ou textuelles**.
...
{gghighlight} est utile ; `gghighlight()` souligne aussi des valeurs dans d’autres tracés.
...
surv %>% 
  ...
  labs(title = "Cas hebdomadaires par hôpital", subtitle = "Port Hospital mis en avant", y = "Cas", x = "")
```
