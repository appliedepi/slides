...
demo %>%                    # ПРИНТ только случаи старше 10 лет
  filter(age > 10)
```
...
```{r, echo=params$lang == "en", eval=FALSE}
demo %>% 
...
```{r, echo=params$lang == "fr", eval=TRUE}
demo %>% 
...
```{r, echo=params$lang == "ru", eval=TRUE}
demo %>% 
  mutate(gender = recode(gender,   # переопределить пол как: 
    'm' = 'male',                  # когда "m", меняется на "male", m меняется на мужской 
    'f' = 'female'))               # когда "f", меняется на "female", f меняется на женский
```
