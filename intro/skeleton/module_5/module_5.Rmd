...
surv %>% 
  group_by(hospital) %>% 
  summarise(
    n_rows  = n(),                               # n() cuenta el número de filas del grupo
    age_avg = mean(age_years, na.rm = T),        # edad media del grupo
    max_onset = max(date_onset, na.rm=T),        # última fecha de inicio
    n_female = sum(gender == 'female', na.rm=T), ## número de mujeres en el grup# Porcentaje de mujeres en el grupoo
    pct_female = percent(n_female / n_rows))     
```
...
```{r dplyr-flex-en, echo=params$lang == "es", eval = params$lang == "es"}
surv %>% 
  group_by(hospital) %>% 
  summarise(
    n_rows  = n(),                               # n() cuenta el número de filas por grupo
