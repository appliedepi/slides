...
```{r , echo=F, eval = params$lang == "ua"}
decorate("summarise2_ua") %>%
  flair("n_rows", color = "deeppink") %>%
  flair("n()", color = "cornflowerblue") %>% 
  knit_print.with_flair()
```
...
```{r summarise4_ua, include = F}
surv %>% 
  group_by(hospital) %>% 
  summarise(
    n_rows  = n(),                               # n() підраховує кількість рядків у групі
    age_avg = mean(age_years, na.rm = T),        # середній вік у групі
    max_onset = max(date_onset, na.rm=T))        # остання дата початку захворювання
...
surv %>% 
  ...
  summarise(
    ...
    age_avg = mean(age_years, na.rm = T),        # середній вік у групі
    max_onset = max(date_onset, na.rm=T),        # остання дата початку захворювання
    n_female = sum(gender == 'female', na.rm=T)) # кількість жінок у групі
...
surv %>% 
  ...
  summarise(
    ...
    max_onset = max(date_onset, na.rm=T),        # остання дата початку захворювання
    n_female = sum(gender == 'female', na.rm=T), # кількість жінок у групі
    pct_female = percent(n_female / n_rows))     # відсоток жінок у групі
```
...
```{r dplyr-flex-au, echo=params$lang == "ua", eval = params$lang == "ua"}
surv %>% 
  group_by(hospital) %>% 
  summarise(
    n_rows  = n(),                                 # n() підраховує кількість рядків у групі
