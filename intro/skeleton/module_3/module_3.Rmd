...
outline %>% 
  ...
  flextable::add_footer_lines(str_glue({paste0(mod1[mod1$uid == "mod3_tb_session_4_1", params$lang])}))
```
...
`r paste0(mod3[mod3$uid == "mod3_tx_dplyR", params$lang])`  
...
`case_when()`|`r paste0(mod3[mod3$uid == "mod3_tb_dplyR_2_5", params$lang])`  
...
demo %>%                   # ПРИНТ только детские случаи
  filter(age < 18)
```
...
```{r mutate2, echo=params$lang == "en", eval=params$lang == "en"}
demo %>% 
...
```{r mutate2, echo=params$lang == "fr", eval=params$lang == "fr"}
demo %>% 
...
```{r mutate2, echo=params$lang == "ru", eval=param$lang == "ru"}
demo %>% 
