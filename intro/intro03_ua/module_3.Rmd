...
```{r, eval=F, echo=params$lang == "ua"}
# повні демонстраційні дані
filter(demo, ___)
```
...
```{r, eval=F, echo=params$lang == "ua"}
#select(), щоб зберегти лише стовпці case_id та age
select(demo, case_id, age)
```
...
```{r, eval=F, echo=params$lang == "ua"}
#видалити стовпці case_id і lab_confirmed із вихідного набору даних
select(demo, -case_id, -lab_confirmed)
```
...
```{r, eval=F, echo=params$lang == "ua"}
demo %>% 
  select(case_id, age, gender, lab_confirmed)        # вибрати стовпці
```
...
demo %>% 
  select(case_id, age, gender, lab_confirmed) %>%    # вибрати стовпці
  distinct() %>%                                     # дедублювати
  filter(age < 18, lab_confirmed == TRUE)            #підтверджені випадки захворювання лише в маленьких дітей
```
...
demo %>% 
  ...
  distinct() %>%                                     # дедублювати
  filter(age < 18, lab_confirmed == TRUE) %>%        #підтверджені випадки захворювання лише в маленьких дітей     
  mutate(infant = ifelse(age < 1, "infant", "not infant"))  # створити стовпець     
...
```{r, eval=params$lang == "ua", echo=params$lang == "ua"}
demo %>% 
  clean_names() %>% 
  rename(
       # НОВИЙ    = СТАРИЙ
...
demo %>%                    #Retorna sólo los casos de niños
  filter(age < 18)
```


```{r, echo=params$lang == "ua", eval=F}
demo %>%                   # ДРУКУВАТИ тільки випадки у дітей
  filter(age < 18)
```
