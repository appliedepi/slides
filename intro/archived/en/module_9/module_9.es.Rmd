...
.nota\[`r paste0(mod9[mod9$uid == "mod9_fn_slides", params$lang])`\]
...
.nota\[`r paste0(mod9[mod9$uid == "mod9_fn_vocab", params$lang])`\]

???
Este proceso ocurre en segundo plano, por lo que no necesitas conocer todos estos pasos. Sin embargo, es posible que te encuentres con estos nombres.
...
???
Ten en cuenta que empieza y acaba con tres guiones, y que la colocación de espacios y dos puntos es muy importante
...
???
Si escribir el documento en código te intimida, RStudio te permite poner el script en "Modo Visual", para que tu documento tenga un aspecto similar al de un documento de Word.
...
???
Consulta en el Manual las opciones para crear una "Fábrica de informes" donde sea más fácil catalogar los resultados de muchos informes diferentes en carpetas con fecha y hora.
...
<!-- `` `r knitr::inline_expr("format(Sys.Date())")``, había -->

<!-- `` `r knitr::inline_expr("nrow(surv)")`` casos confirmados.   -->

<!-- A partir de -->

<!-- `` `r knitr::inline_expr("format(Sys.Date(), '%d %B, %Y'"))``, había -->

<!-- `` `r knitr::inline_expr("nrow(surv)")` `` casos confirmados.   -->

<!-- Produce esto:   -->

<!-- A partir de `r format(Sys.Date(), '%d %B, %Y')`, había `r nrow(surv)` casos confirmados.   -->
