...
.footnote[[source de l'image](https://rmarkdown.rstudio.com/authoring_quick_tour.html)]
...
.panelset[
.panel[.panel-name[Pour obtenir ceci]
Une phrase normale.  
*Voici un texte à mettre en italique*      
**Voici un texte à mettre en gras**   

* Point 1 
* Point 2 

]
.panel[.panel-name[Tapez ceci]
Une phrase normale.  
`*Voici un texte à mettre en italique*   `   
`**Voici un texte à mettre en gras**   `
...
.footnote[[Pour en savoir plus sur le formatage RMarkdown, cliquez ici.](https://rmarkdown.rstudio.com/authoring_basics.html)]
...
pacman::p_load(
  ...
  tidyverse # donnees management & viz
)
...
epikit::fmt_count(surv, is.na(date_symptomes))
```

```{r, eval=params$lang == "en", echo=params$lang == "en"}
str_glue("{fmt_count(surv, is.na(date_symptomes))} are missing date of onset and not shown.")
```

```{r, eval=params$lang == "fr", echo=params$lang == "fr"}
str_glue("{fmt_count(surv, is.na(date_symptomes))} cas n'ont pas de date d'apparition et ne sont pas présentés ici.")
```

```{r, eval=params$lang == "ru", echo=params$lang == "ru"}
str_glue("{fmt_count(surv, is.na(date_symptomes))} случаев дата начала заболевания отсутствует и не показана.")
```

```{r, eval=params$lang == "es", echo=params$lang == "es"}
str_glue("{fmt_count(surv, is.na(date_symptomes))} falta la fecha de inicio y non se muestra.")
```
