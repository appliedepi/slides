...
pacman::p_load(
  ...
  tidyverse,  # data manedadment and visualization
...
- Importar los datos de una subcarpeta del proyecto utilizando `import()` y `here()`
...
```{r, echo=T, eval=F}
import("data/raw/surveillance_linelist_20141201.csv") # funciona casi en cualquier ordenador
```
...
*El operador de asignación `<-` guarda el conjunto de datos como un objeto con el nombre `surv_raw`.*
...
.nota["dplyr" es la abreviatura de "data plier" (pinza de datos): la pinza en el logo de arriba]
...
Vamos a utilizar una mini base de datos, **`surv_raw`**, para practicar algunas funciones básicas de R.
...
surv_raw <- data.frame(
  stringsAsFactors = FALSE,
       check.names = FALSE,
         ...
         sexo = c("h", "m", "h", "m", "m"), 
         confirm_lab = c(FALSE, TRUE, TRUE, TRUE, FALSE),
...
```{r, eval=T, echo=F}
filter(surv_raw, edad < 18) %>% knitr::kable()
```
...
```{r, eval=F, echo=T}
filter(surv_raw, edad < 18, sexo == "f")
```
...
```{r, eval=T, echo=F}
filter(surv_raw, edad < 18 & (sexo == "f" | confirm_lab == TRUE)) %>%
  knitr::kable()
```
...
```{r, eval=F, echo=T}
select(surv_raw, id_caso, edad, sexo)
```
...
```{r, eval=F, echo=T}
select(surv_raw, -id_caso, -confirm_lab)
```
...
`filter(`**surv\_raw**`, edad < 18)`

--

Usando tuberías, esto se escribe ahora como

**surv\_raw** `%>% filter(edad < 18)`
...
Puedes conectar los datos de `surv_raw` a través de *múltiples* funciones  
`surv_raw` **%>%** `filter(edad < 18)`

```{r, eval=T, echo=F}
surv_raw %>% filter(edad < 18) %>% knitr::kable()
```
...
Puedes conectar los datos de `surv_raw` a través de *múltiples* funciones  
`surv_raw` **%>%** `filter(edad < 18)` **%>%** `select(id_caso, edad, sexo)`

```{r, eval=T, echo=F}
surv_raw %>% filter(edad < 18) %>% select(id_caso, edad, sexo) %>% knitr::kable()
```
...
surv_raw %>% 
  select(id_caso, edad, sexo, confirm_lab)                  # seleccionar columnas
```
...
surv_raw %>% 
  select(id_caso, edad, sexo, confirm_lab)        # выбрать столбцы
```
...
surv_raw %>% 
  #add_row(surv_raw[5,]) %>% 
  select(id_caso, edad, sexo, confirm_lab) %>%    # select columns
  knitr::kable() 
```
...
surv_raw %>% 
  add_row(surv_raw[5,]) %>% 
  select(id_caso, edad, sexo, confirm_lab) %>%    # sélectionner les colonnes
  knitr::kable() 
```
...
surv_raw %>% 
  #add_row(surv_raw[5,]) %>% 
  select(id_caso, edad, sexo, confirm_lab) %>%    # des-duplicar
  knitr::kable() 
```
...
surv_raw %>% 
  select(id_caso, edad, sexo, confirm_lab) %>%              # select columns
  distinct() %>%                                            # de-duplicate
  filter(edad < 18, confirm_lab == TRUE)                   # only children cases
```
...
surv_raw %>% 
  select(id_caso, edad, sexo, confirm_lab) %>%    # Sélectionner les colonnes
  distinct() %>%                                     # dédupliquer 
  filter(edad < 18, confirm_lab == TRUE)            # uniquement les cas confirmés d'enfants 
```
...
surv_raw %>% 
  select(id_caso, edad, sexo, confirm_lab) %>%    # выбрать столбцы
  distinct()  %>%                                    # де-дублировать
  filter(edad < 18, confirm_lab == TRUE)            #подтвержденные случаи заболевания только у маленьких детей
```
...
surv_raw %>% 
  select(id_caso, edad, sexo, confirm_lab) %>%    # seleccionar columnas
  distinct() %>%                                     # des-duplicar
  filter(edad < 18, confirm_lab == TRUE)            # sólo los casos confirmados de niños pequeños
```
...
surv_raw %>% 
  ...
  filter(edad < 18, confirm_lab == TRUE) %>%        # only children confirmed cases
  knitr::kable() 
```
...
surv_raw %>% 
  ...
  distinct() %>%                                            # de-duplicate
  filter(edad < 18, confirm_lab == TRUE) %>%               # only children cases      
  mutate(infant = ifelse(edad < 1, "infant", "not infant"))  # create a column      
...
surv_raw %>% 
  ...
  distinct() %>%                                     # dédupliquer 
  filter(edad < 18, confirm_lab == TRUE) %>%        # uniquement les cas confirmés d'enfants
  mutate(infant = ifelse(edad < 1, "infant", "not infant")) # créer une colonne       
...
surv_raw %>% 
  select(id_caso, edad, sexo, confirm_lab) %>%    # выбрать столбцы
  distinct()  %>%                                    # де-дублировать
  filter(edad < 18, confirm_lab == TRUE) %>%        # подтвержденные случаи заболевания только у детей младшего возраста                   
  mutate(infant = ifelse(edad < 1, "infant", "not infant"))  # создать столбец
...
surv_raw %>% 
  select(id_caso, edad, sexo, confirm_lab) %>%    # seleccionar columnas
  distinct() %>%                                     # des-duplicar
  filter(edad < 18, confirm_lab == TRUE) %>%        # casos confirmados sólo en niños pequeños      
  mutate(infant = ifelse(edad < 1, "infant", "not infant"))  # crear una columna      
...
surv_raw %>% 
  #add_row(surv_raw[5,]) %>% 
  select(id_caso, edad, sexo, confirm_lab) %>%          
  distinct() %>%                                           
  filter(edad < 18, confirm_lab == TRUE) %>%              
  mutate(infant = ifelse(edad < 1, "infant", "not infant")) %>% 
  knitr::kable()
```
...
De forma equivalente, `surv_raw` puede pasarse a `names()` utilizando un **pipe**:

```{r, echo=T, eval=F}
surv_raw %>%  # comienza con los datos crudos
...
Aplica `clean_names()` a `surv_raw` agregándolo en la secuencia.  
...
Los cambios en `surv_raw` **no** fueron guardados.
...
¡**`surv_raw`** todavía tiene los *nombres originales de las columnas*!
...
Utiliza el operador **`<-`** para guardar los cambios en un nuevo objeto **`surv_clean`**.
...
¡**`surv_clean`** tiene ahora los *nombres de las columnas limpios*!
