...
*WARNING*: **Categorizing** magnitude tends to not be **statistical**, especially when there is limited historical data.

There is no objective measure of how much infection or disease is "too much" in a population.

The population, health authorities, and political class together determine what an acceptable level of infection/disease is. 

If a society chooses an elimination and/or supression strategy, then no amount of disease spread will be acceptable.

**First** present your data in a neutral manner with scientific integrity.

- Show the reader the real numbers (**don't categorize into low/medium/high**).
- Be careful of displaying multiple time series with different population sizes (e.g. 100 deaths / 1 million vaccinated vs 10 deaths / 10 unvaccinated). You may want to display rates (i.e. deaths / 100k people).

**Then** you may interpret (or leave that to the reader).
...
pd <- timeseries %>% 
  ...
  filter(isoyear >= 2020)
data.table::setDT(pd)
cstidy::set_csfmt_rts_data_v1(pd)
pd <- csalert::short_term_trend(
  pd,
  numerator = "consultations_icpc2group_n",
  include_decreasing = F
)
cstidy::remove_class_csfmt_rts_data(pd)
pd <- pd[isoyear %in% 2021:2022]
...
q <- q + geom_text(
  data = pd[consultations_icpc2group_trend0_41_status=="increasing"],
  mapping = aes(
    y = 0,
    label = "▲"
  ),
